...
class MotivApi {
  ...
  getLastAwakening() {
    return this.getLastSleepEvent().then((sleepEvent) => {
      return new Date((sleepEvent.utcEnd - timeZoneOffset) * milliseconds);
    });
  }
  ...
  getLastHeartRateRecord() {
    ...
    return new Promise((resolve, reject) => {
      ...
      if (self.lastHeartRateRecord && !self.isCallTime(self.lastCallTimes.heartRate)) {
      ...
      securePost(
        ...
        if (
          ...
          self.lastCallTimes.heartRate = Date.now();
        } else {
  ...
  getLastSleepEvent() {
    ...
    return new Promise((resolve, reject) => {
      ...
      if (self.lastSleepEvent && !self.isCallTime(self.lastCallTimes.sleepEvent)) {
      ...
      securePost(motivApiHost, motivPaths.sleepEvent, sleepRequestData, self.authenticatedHeaders)
        .then((sleepResultData) => {
          if (sleepResultData && sleepResultData.results && sleepResultData.results.length > 0) {
            ...
            self.lastCallTimes.sleepEvent = Date.now();
          } else {
            self.lastSleepEvent = { utcEnd: timeZoneOffset, sleepMinutes: 0 };
          }
          resolve(self.lastSleepEvent);
        })
        .catch(reject);
  ...
  getUserProfile() {
    ...
    return new Promise((resolve, reject) => {
      ...
      if (self.profileData && !self.isCallTime(self.lastCallTimes.userProfile)) {
      ...
      securePost(
        ...
        .then((profileResultData) => {
          ...
          self.lastCallTimes.userProfile = Date.now();
          resolve(self.profileData);
        })
        .catch(reject);
    });
