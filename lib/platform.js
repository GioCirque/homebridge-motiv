...
const { getUTCNowDate, splitDateAndTime, toTitleCase } = require('./utils');
...
class MotivPlatform {
  constructor(log, config, api) {
    ...
    this.PlatformAccessory = this.PlatformAccessory;
  ...
  updateSensors() {
    try {
      this.accessories.forEach((a) => {
        if (a && a.hasOwnProperty('update') && typeof a.update === 'function') {
          a.update();
        }
      });
    } catch (err) {
    ...
    try {
      this.accessories.forEach((a) => {
        if (a.context.type === type) {
          a.getService(this.serviceType).setCharacteristic(characteristic, value);
        }
      });
    } catch (err) {
      this.log.error(err);
  ...
  setup() {
    try {
      if (this.motivApi.needsAuth === false) {
        setInterval(this.updateSensors, this.motivSyncInterval);
        try {
  ...
  configureAccessory(accessory) {
    try {
      if (!this.accessories.has(accessory.UUID)) {
        ...
        switch (type) {
          case 'awake':
            accessoryImpl = new SleepSensor(this, uuid, accessory);
            break;
          default:
        ...
        this.accessories.set(accessory.UUID, accessoryImpl.accessory);
      }
    } catch (err) {
      this.log.error(err);
    }
  ...
  addAccessory(accessoryName) {
    try {
      ...
      if (!this.accessories.has(uuid)) {
        ...
        switch (type) {
          case 'awake':
            accessory = new SleepSensor(this, uuid);
            break;
          default:
