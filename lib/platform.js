  ...
  constructor(log, config, api) {
    ...
    this.api.on('didFinishLaunching', () => {
      ...
      if (!self.config.account) {
        ...
        self.log.info('Setting up Motiv platform specifics and updating sensors.');
        self.setup();
        self.updateSensors();
      }
    });
  ...
  setup() {
    try {
      if (this.motivApi.needsAuth === false) {
        setInterval(this.updateSensors, this.motivSyncInterval, this);
        try {
