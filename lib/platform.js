  ...
  updateSensors() {
    try {
      this.accessories.forEach((a) => {
        this.log.info(`Checking '${a.accessory.displayName}' for potential update`);
        if (a && a.hasOwnProperty('update') && typeof a.update === 'function') {
          this.log.info(`Device '${a.accessory.displayName}' is updatable`);
          a.update();
        } else {
          this.log.error(`Device '${a.accessory.displayName}' is NOT updatable`);
          this.log.error(
            `Device '${a.accessory.displayName}' reports hasUpdate: ${a.hasOwnProperty(
              'update'
            )}, updateIs: ${typeof a.update}`
          );
        }
      });
    } catch (err) {
